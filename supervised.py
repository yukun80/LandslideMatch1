...
def evaluate(model, loader, mode, cfg):
    model.eval()  # 将模型设置为评估模式，这意味着模型的权重不会被更新。
...
def main():
    ...
    rank, world_size = setup_distributed(port=args.port)  # 建立数据并行性是为了跨多个 GPU 同步操作和模型状态。
    ...
    """优化器设置和模型初始化。我们使用 SGD 优化器，其中包含两个参数组。第一个参数组用于优化骨干网络的权重，而第二个参数组用于优化其他所有参数。"""
    optimizer = SGD(
        [
    ...
    ProbOhemCrossEntropy2d 是一种基于概率的硬样本挖掘损失函数，它可以有效地处理不平衡的数据集。"""
    ...
    """数据集设置。我们使用 SemiDataset 类来加载数据集。在训练期间，我们使用 train_l 数据集，其中包含标记数据。在评估期间，我们使用 val 数据集。"""
